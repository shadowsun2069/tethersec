...
# Shell enhancements for Termux on Android
# Source this file: source /data/data/com.termux/files/home/.hermes/scripts/termux-shell-config.sh
...
alias cat='bat -p'   # -p = plain (no header), good for piping
alias find='fd'
alias vim='micro'
alias vi='micro'

# --- Termux/Android shortcuts ---
...
alias android='cd /storage/emulated/0/Android'
alias batstatus='termux-battery-status | jq'

# --- Clipboard helpers ---
alias clipget='termux-clipboard-get'
alias clipset='termux-clipboard-set'
alias notify='termux-notification --title Hermes'
...
# --- Prompt with zoxide indicator ---
# Shows current directory and a small zoxide hint if applicable
PS1='\[\e[0;32m\][Termux] \[\e[0;36m\]\w\[\e[0m\] $ '
...
termux_welcome() {
    echo ""
    echo "  Tether | Samsung S26 Ultra | $(date +%H:%M)"
    echo "──────────────────────────────────────────────"
    # battery
    bat_json=$(termux-battery-status 2>/dev/null)
    if [ -n "$bat_json" ]; then
        pct=$(echo "$bat_json" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('percentage','?'))" 2>/dev/null)
        status=$(echo "$bat_json" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('status','?'))" 2>/dev/null)
        temp=$(echo "$bat_json" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('temperature','?'))" 2>/dev/null)
        echo "  Battery: ${pct}% | ${status} | ${temp}°C"
    fi
    # uptime
    up=$(uptime -p 2>/dev/null | sed 's/up //')
    echo "  Uptime:  ${up}"
    # storage
    echo "  Storage: $(df -h /storage/emulated | awk 'NR==2{print $3" used / "$4" free"}')"
    # todos
    todo_count=$(export PATH="$HOME/.local/bin:$PATH"; nb todo 2>/dev/null | wc -l)
    echo "  Todos:   ${todo_count} pending"
    # hermes version (best effort)
    if [ -f "$HOME/.hermes/hermes-agent/venv/bin/hermes" ]; then
        hv=$(hermes --version 2>/dev/null | head -1)
        echo "  Hermes:  ${hv}"
    fi
    echo "──────────────────────────────────────────────"
    echo "  nb add | nb todo add | nb search | nb sync"
    echo "  tmux | fzf | z | bat | eza | micro | atuin"
    echo ""
}

# Run welcome on interactive shells only when stdin/stdout are real TTYs
# (avoids hanging Hermes init_session which runs bash -l -c without a TTY)
